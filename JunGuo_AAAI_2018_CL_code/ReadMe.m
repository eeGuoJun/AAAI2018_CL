%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Descripion: 
% This source code is for our paper
% Jun Guo and Wenwu Zhu, "Partial Multi-view Outlier 
% Detection Based on Collective Learning," In AAAI 2018.
%   -Source code version 1.0  2018/03/01 by Jun Guo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Note 1: 
% More source codes will be released after tidying up. 
% Please refer to our paper for detailed experimental settings.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Please cite our work if you find the code helpful.
% @inproceedings{JunGuo_AAAI_2018_CL,
%   author = {J. Guo and W. Zhu},
%   title = {Partial Multi-view Outlier Detection Based on Collective Learning},
%   booktitle = {Proc. AAAI Conf. Artificial Intell. (AAAI)},
%   address = {New Orleans, Louisiana},
%   pages = {298-305},
%   month = {Feb.},
%   year = {2018}
% }
% If you have problems, please contact us at eeguojun@outlook.com
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
